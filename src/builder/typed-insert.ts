...
export class TypedInsertBuilder<DB, TB extends keyof DB> {
   ...
   * value for a column declared `GENERATED ALWAYS AS IDENTITY`. PG-only
   * (SQL:2003). MySQL/SQLite have no equivalent; MSSQL uses the
   * separate `SET IDENTITY_INSERT` statement.
   */
  overridingSystemValue(): TypedInsertBuilder<DB, TB> {
    return this._withBuilder(this._builder.overridingSystemValue())
  }

  /**
   * `OVERRIDING USER VALUE` — force the system-generated value even
   * when the user supplied one for a `GENERATED BY DEFAULT AS
   * IDENTITY` column. PG-only (SQL:2003).
   */
  overridingUserValue(): TypedInsertBuilder<DB, TB> {
    return this._withBuilder(this._builder.overridingUserValue())
  }
