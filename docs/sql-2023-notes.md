...
- **`OVERRIDING { SYSTEM | USER } VALUE`** — SQL:2003 clause that sits between the column list and `VALUES` / `SELECT`. `OVERRIDING SYSTEM VALUE` lets an INSERT provide a value for a column declared `GENERATED ALWAYS AS IDENTITY` (which normally rejects user-supplied values); `OVERRIDING USER VALUE` forces the system value even if the user supplied one for a `GENERATED BY DEFAULT AS IDENTITY` column. Builder methods: `.overridingSystemValue()` / `.overridingUserValue()` on both the untyped `InsertBuilder` and the typed `TypedInsertBuilder`. AST: new `InsertNode.overriding?: "SYSTEM" | "USER"` slot. Dialect matrix: PG only (10+). MSSQL has the separate `SET IDENTITY_INSERT <table> ON` _statement_ with similar effect for the SYSTEM case (no USER analogue) — silently rewriting one into the other would surprise callers, so the MSSQL printer throws via the `INSERT_OVERRIDING` feature flag. MySQL and SQLite have no equivalent grammar and their auto-increment columns accept user values directly, so the clause is also semantically unnecessary there; both throw. ✅
